...
alloc_proc(void) {
	 ...
	 */
     //LAB6 2016010981 : (update LAB5 steps)
    /*
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = copy_files(clone_flags, proc);     //    2.5. call copy_files to copy opened files to child process
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2016010981  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    //(3.1) read ELF header from disk
    struct elfhdr elf_header, *elf = &elf_header;
    ret = load_icode_read(fd, elf, sizeof(struct elfhdr), 0);
    if (ret != 0) goto bad_elf_cleanup_pgdir;
    ...
    int prog_num;
    for (prog_num = 0; prog_num < elf->e_phnum; ++prog_num) {
    //(3.4) find every program section headers
        off_t prog_header_offset = elf->e_phoff + prog_num * sizeof(struct proghdr);
        ret = load_icode_read(fd, ph, sizeof(struct proghdr), prog_header_offset);
        if (ret != 0) goto bad_cleanup_mmap;
    ...
    //(3.5) call mm_map fun to setup the new vma ( ph->p_va, ph->p_memsz)
        ...
        size_t off, size;
        off_t section_offset = ph->p_offset;
     ...
     //(3.6.1) copy TEXT/DATA section of bianry program
        while (start < end) {
            ...
            // copy from disk
            ret = load_icode_read(fd, page2kva(page) + off, size, section_offset);
            if (ret != 0) goto bad_cleanup_mmap;
            start += size;
            section_offset += size;
        }
    ...
    //(6) setup uargc and uargv in user stacks
    // since CR3 is loaded with new pgdir, now user stack is accessible
    uintptr_t stack = USTACKTOP;
    // temporarily store pointers to arguments
    char *uargv[EXEC_MAX_ARG_NUM];
    uargv[argc] = NULL; // mark end of arguments
    // copy arguments to user stack
    int i = argc;
    while (i --> 0) {
        size_t str_size = strlen(kargv[i]) + 1;
        stack -= str_size;
        uargv[i] = (char *) stack;
